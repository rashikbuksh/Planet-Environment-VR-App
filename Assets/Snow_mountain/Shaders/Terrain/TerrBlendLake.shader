Shader "Nature/Terrain/TerrBlend Lake" {
Properties {
	...
	_SnowHeight ("SnowHeight", Range (-1.6, 0.5)) = 0.3
	
	_ColorTex ("ColorMap (RGB)", 2D) = "black" {}
	_Reflectmap ("Reflectmap (RGB)", 2D) = "black" {}
	_Normalmap ("Normalmap (RGB)", 2D) = "white" {}
	_StripesTex ("StripesTex (RGB)", 2D) = "black" {}
	_Tiling ("Tiling", Range (0.01, 1)) = 0.05
...
void surf (Input IN, inout SurfaceOutput o) {
			...
			IN.uv_Splat2 += offset3/4;

			half4 ColorTex = tex2D (_ColorTex, IN.uv_Control);
			half4 Reflectmap = tex2D (_Reflectmap, IN.uv_Control);
			half4 StripesTex = tex2D (_StripesTex, IN.worldPos.xy /5 * _Tiling);
			...
			half4 Detail1 = tex2D (_Splat1, IN.uv_Splat1) * ColorTex * StripesTex;
			...
			half4 HeightSplatTex4 = tex2D (_HeightSplatAll, IN.uv_Splat3 - offset).a + _SnowHeight; 
			...
			o.Gloss = tex.a;
			o.Alpha = tex.r * _Color.a;
			o.Specular = _Shininess;
			
	float3 worldRefl = WorldReflectionVector (IN, UnpackNormal(mixnormal));
	fixed4 reflcol_dirt = texCUBE (_Cube, worldRefl)*4;
	reflcol_dirt *= ColorTex.a;
	reflcol_dirt *= tex2D(_Reflectmap, IN.uv_Control).r;
	reflcol_dirt *= tex2D(_Reflectmap, IN.uv_Splat2).g;
	reflcol_dirt *= b2;
	
	fixed4 reflcol_snow = texCUBE (_Cube, worldRefl)*80;
	reflcol_snow *= ColorTex.a;
	reflcol_snow *= (tex2D(_Reflectmap, IN.uv_Splat3).b * tex2D(_Reflectmap, IN.uv_Splat3/1.7 + _Time/800).b);
	reflcol_snow *= b3;
	o.Emission = reflcol_dirt.rgb + (reflcol_snow.rgb * _ReflectColor.rgb);
}
